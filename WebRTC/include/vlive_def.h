...
namespace vlive {
   ...
   enum VHStreamType {
      VHStreamType_PureAudio = 0,
      VHStreamType_Preview_Video = 1,             //预览视频数据，图片采集
      ...
      VHStreamType_Auxiliary_CamerCapture = 6,    //双推摄像头
      VHStreamType_Count
   };
   ...
   enum RtcRoomNetState
   {
      ...
      VHRoomConnect_ROOM_RE_CONNECTED,       //网络重连已经恢复
   ...
   enum VHCapture {
      ...
      VHCapture_CAMERA_LOST,          //视频拔出
      VHCapture_STREAM_SOURCE_LOST,   //摄像设备采集过程中采集失败 ,SDK将结束采集与推流
      VHCapture_PLAY_FILE_ERR,        //播放文件失败
      VHCapture_AUDIODEV_REMOVED,     //音频设备移出
      VHCapture_AUDIO_CAPTRUE_ERR,    //音频设备初始化或采集失败
      VHCapture_VIDEO_CAPTURE_ERROR,  //视频采集5秒钟之内没有采集到画面事件
   };
